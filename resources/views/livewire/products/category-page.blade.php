...
// 2. STATE (Değişkenler):
state(['category']); // Kategori objesi burada tutulacak
state(['selectedSizes' => []]);
...
// DİKKAT: Buradaki değişken adını ($categories) rotadaki {categories} ile aynı yaptık.
mount(function ($categories) {
    
    // 1. ADIM: Kategoriyi slug'a göre veritabanından bul.
    // Bulamazsa otomatik 404 hatası ver (firstOrFail).
    $this->category = Category::where('slug', $categories)->firstOrFail();

    // Sayfa başlığını ayarla (Artık $this->category dolu olduğu için hata vermez)
    title($this->category->name);

    // 2. ADIM: Varyantları ve Filtreleri Hazırla
    // Bu kategoriye ait aktif ürünlerin ID'lerini al
    $productIds = $this->category->products()->pluck('id');

    // O ürünlere ait stoktaki varyantları çek
    $variants = ProductVariant::whereIn('product_id', $productIds)
                               ->where('stock', '>', 0)
                               ->get();

    // Filtre seçeneklerini doldur
    $this->availableSizes = $variants->pluck('size')->filter()->unique()->sort()->values();
    $this->availableColors = $variants->pluck('color_name')->filter()->unique()->sort()->values();
});

// 4. WITH (Veri Çekme):
with(fn () => [
    'products' => $this->category->products() // İlişki üzerinden git
